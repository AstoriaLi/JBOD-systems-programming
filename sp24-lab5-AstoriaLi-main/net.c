...
static bool nread(int fd, int len, uint8_t *buf) {
  ...
  int curr = 0;
  while(total < len){
    curr = read(fd, buf + total, len - total);
...
static bool nwrite(int fd, int len, uint8_t *buf) {
  ...
  int curr = 0;
  while(total < len){
    curr = write(fd, buf + total, len - total);
...
// help function to create the packet
uint8_t * create_packet(uint16_t length, uint32_t opCode, uint16_t returnCode, uint8_t *block, uint8_t *packetPtr){
  ...
  // update the values
  memcpy(packetPtr, &length, 2);
  memcpy(packetPtr + 2, &opCode, 4);
  memcpy(packetPtr + 6, &returnCode, 2);
  
  if (packet_size == 264){
    memcpy(packetPtr + 8, block, JBOD_BLOCK_SIZE);
  }
...
static bool send_packet(int sd, uint32_t op, uint8_t *block) {
  ...
  if(((op >> 14) & 0b111111) == JBOD_WRITE_BLOCK){
    len += 256;
  }
  
  uint8_t packet[len];
  create_packet(len, op, code, block, packet);
...
int jbod_client_operation(uint32_t op, uint8_t *block) {
  ...
  recv_packet(cli_sd, &op, &val, block);
